...
class Constraint:
    ...
    def __init__(self,
                 ...
                 agent: int,
                 step: int,
                 loc_1: typing.Optional[npt.NDArray[int]],
                 loc_2: typing.Optional[npt.NDArray[int]] = None,
...
class ConstraintTable:
    ...
    def is_constrained(self, current_loc: npt.NDArray[int], next_loc: npt.NDArray[int], step: int) -> bool:
        for c in self.constraints:
            ...
            if c.positive: # positive constraint
                if np.any(next_loc - c.loc_1 != 0):
                    return True
            elif c.loc_2 is None: # negative vertex constrain
                if np.all(next_loc - c.loc_1 == 0):
                    return True
            elif np.all(current_loc - c.loc_1 == 0) and np.all(next_loc - c.loc_2 == 0): # negative edge constraint
                return True
        return False
